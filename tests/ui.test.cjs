...
async function scenario(resume=false){
 ...
 w.eval(script);
 await until(()=>w.document.querySelector('#liveAuditTitle').textContent.includes('проверка завершена'));
 await until(()=>!w.document.querySelector('#collectButton').disabled);
 assert.equal(calls.length,resume?0:1,'startup resumes existing job or starts automatic loading');
 if(!resume)assert.equal(calls[0].force,false);
 assert.match(w.document.querySelector('#companyProgress').textContent,/HTTP 503/);
 assert.match(w.document.querySelector('#comparisonTable').textContent,/1\s*300\s*₽/);

 assert.equal(w.document.querySelector('#liveModeSelect').value,'mixed','fresh install shows LIVE plus confirmed user documents');
 assert.equal(w.document.querySelector('#exactCount').textContent,'1','counter follows LIVE filter');
 assert.equal(w.document.querySelector('#missingCount').textContent,'1');
 ...
 await until(()=>!w.document.querySelector('#collectButton').disabled);
 w.document.querySelector('#swapRouteButton').click();await until(()=>calls.at(-1).origin==='Москва');
 ...
 await until(()=>calls.at(-1).profile==='w200');
 await until(()=>!w.document.querySelector('#collectButton').disabled);
 ...
 await until(()=>calls.at(-1).destination==='Казань');await until(()=>!w.document.querySelector('#collectButton').disabled);
 ...
 await until(()=>calls.at(-1).origin==='Екатеринбург');await until(()=>!w.document.querySelector('#collectButton').disabled);
 assert.equal(calls.at(-1).destination,'Казань');
 w.document.querySelector('#swapRouteButton').click();await until(()=>calls.at(-1).origin==='Казань');await until(()=>!w.document.querySelector('#collectButton').disabled);
 ...
 w.document.querySelector('#autoRefreshToggle').checked=false;
 w.document.querySelector('#autoRefreshToggle').dispatchEvent(new w.Event('change'));
...
(async()=>{await scenario();await scenario(true);console.log('PASS: auto loading, progress and errors, retry failed and single company, LIVE counters, manual reload, route and profile changes, empty selection, resume without duplicate job');})().catch(e=>{console.error(e);process.exit(1);});
