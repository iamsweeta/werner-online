...
(async()=>{
 ...
 assert.equal($('importApplyFeedback').hidden,false);assert.match($('importApplyFeedback').textContent,/применён/);assert.equal($('importDoneButton').hidden,false);assert.equal($('importApplyButton').textContent,'Цены применены');assert.equal(commitCount,1);assert.equal($('onlineCount').textContent,'0');assert.equal($('liveModeSelect').value,'all');
