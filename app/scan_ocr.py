...
TIMEOUT=600
STAGE_TIMEOUT=120
WARNING='Цены прочитаны со скана (OCR). Числа проверены повторным чтением или сравнением контуров с подтверждёнными символами этого же документа; ошибки распознавания всё равно возможны. Проверьте маршрут, дату и цены по страницам оригинала перед подтверждением.'
...
def prepare(raw,company,origin=None,destination=None):
    ...
    cached=ocr_cache.get(raw,company,origin,destination)
    if cached is not None:
        if cache is not None:cache[key]=cached
        if _PROGRESS.get():_PROGRESS.get()({'message':'Найдено сохранённое распознавание этого файла','done':1,'total':1})
        return cached
...
def parse(result,origin,destination):
    ...
    return values,{'parser':'ДЛ · скан межтерминальной таблицы · OCR','source_page':row['source_page'],
                  ...
                  'route_verified':True,'ocr':True,'ocr_pages':result['ocr_pages'],'ocr_cells':row['ocr_cells'],'ocr_cache_hit':bool(result.get('ocr_cache_hit')), 
                  'calculation_basis':'OCR: числа проверены повторным чтением или сравнением контуров с подтверждёнными символами документа; требуется проверка по оригиналу. ДЛ: фиксированные суммы до 35 кг (объём до 0,1 м³), далее max(минимум, вес × руб/кг). До 10 000 кг, без адресной доставки и допуслуг.'}
